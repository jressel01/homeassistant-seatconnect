...
class SeatConnectConfigFlow(config_entries.ConfigFlow, domain=DOMAIN):
    ...
    async def async_step_login(self, user_input=None):
        ...
        try:
            ...
            return self.async_abort(reason="Failed to connect to Seat Connect")
    ...
    async def async_step_get_vehicles(self, user_input=None):
        ...
        for vehicle in self._connection.vehicles:
            _LOGGER.info(f"Found data for VIN: {vehicle.vin} from Seat Connect")
    ...
    async def async_step_reauth(self, entry) -> dict:
        """Handle initiation of re-authentication with Seat Connect."""
        self.entry = entry
        return await self.async_step_reauth_confirm()

    async def async_step_reauth_confirm(self, user_input: dict = None) -> dict:
        """Handle re-authentication with Seat Connect."""
        errors: dict = {}
   ...
   # Configuration.yaml import
    async def async_step_import(self, yaml):
        ...
        if len(self._connection.vehicles) == 0:
            return self.async_abort(reason="Seat Connect account didn't return any vehicles")
        self._init_info["CONF_VEHICLES"] = {
            vehicle.vin: vehicle.dashboard().instruments
            for vehicle in self._connection.vehicles
        }
    ...
    def async_get_options_flow(config_entry):
        ...
        return SeatConnectOptionsFlowHandler(config_entry)


class SeatConnectOptionsFlowHandler(config_entries.OptionsFlow):
    """Handle SeatConnect options."""
